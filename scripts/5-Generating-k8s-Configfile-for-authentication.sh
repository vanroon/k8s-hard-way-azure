...
export KUBERNETES_PUBLIC_ADDRESS=$(az network public-ip show -g kubernetes \
...
# copy the right kubelet and kube-proxy kubeconfig files to each worker instance
for instance in worker-0 worker-1; do
  PUBLIC_IP_ADDRESS=$(az network public-ip show -g kubernetes \
    ...
    scp ${instance}.kubeconfig kube-proxy.kubeconfig kuberoot@${PUBLIC_IP_ADDRESS}:~/
		...
		# Copy the right kube-controller-manager and kube-scheduler files to each controller
		# instance
